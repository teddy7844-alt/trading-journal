...
class SMCAdvancedReversalEngine:
    ...
    def _build_features(self, df_ltf, df_htf):
        ...
        df_htf = df_htf.copy()
        df_htf['Highest_HTF_High'] = df_htf['High'].rolling(self.htf_lookback).max()
        df_htf['Lowest_HTF_Low'] = df_htf['Low'].rolling(self.htf_lookback).min()
